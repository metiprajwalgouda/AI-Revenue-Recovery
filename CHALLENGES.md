...
Design fix: CheckoutEvent required fields that don't exist for real data. true_reason and payment_method_attempted were required fields — fine for synthetic data (we control ground truth), but a real live abandonment has no known "true reason" (that's what the classifier infers) and may not have a selected payment method yet. Made both optional, confirmed no existing tests broke since synthetic data always sets them explicitly anyway.

Known limitation, documented not hidden: the live storefront currently can't capture otp_requested/otp_verified signals from Razorpay Checkout.js the way the synthetic dataset simulates them — real signal capture is genuinely more limited than synthetic data. This means real abandonments lean more heavily on the LLM fallback path than the synthetic batch does, which is an honest, expected trade-off, not a bug.

Bug: TemplateResponse API changed between starlette versions. The older TemplateResponse(name, {"request": request}) call style silently shifted arguments in the installed starlette version (which expects TemplateResponse(request, name, context)), causing a cryptic TypeError: unhashable type: 'dict' with no obvious connection to the real cause. Fixed by using the current signature, with a regression test confirming / renders successfully so this can't silently break again.


Feature: full multi-tenant auth for merchants and customers. Added separate account systems (MerchantUser, CustomerUser) with bcrypt-hashed passwords and signed session cookies. Critically, merchant and customer tokens use different cryptographic signing salts, not just different cookie names — a merchant token is cryptographically invalid if presented as a customer token, even if someone tampers with cookie values directly. Also added ownership checks so one merchant can't edit another's products, and one customer can't abandon/complete another's checkout session — both return 404 rather than 403 to avoid confirming a resource's existence to a non-owner.